...
public sealed class SqlHelper
{
    ...
    /// <summary>
    /// 调用分页存储过程Pagination,获取分页数据和全部记录数
    /// </summary>
    /// <param name="tableName">数据表名称，多表联查是需在这里拼接join语句</param>
    /// <param name="fieldStr">需要获取的字段名称，以,连接的字符串</param>
    /// <param name="orderStr">排序字段名 不需要ORDER BY 关键字</param>
    /// <param name="orderType">排序类型</param>
    /// <param name="whereStr">查询条件不需要WHERE关键字</param>
    /// <param name="pageSize">分页大小</param>
    /// <param name="pageIndex">当前页面</param>
    /// <param name="totalRecord">返回的查询全部记录数，定义时需要加out参数</param>
    /// <returns></returns>
    public static DataSet GetPagination(string tableName, string fieldStr, string orderStr, string orderType, string whereStr, int pageSize, int pageIndex, out int totalRecord)
    {
        SqlParameter[] paras = new SqlParameter[] {
            new SqlParameter("@TableName",SqlDbType.VarChar,4000),
            new SqlParameter("@ReFieldsStr",SqlDbType.VarChar,1000),
            new SqlParameter("@OrderString",SqlDbType.VarChar,200),
            new SqlParameter("@OrderType",SqlDbType.VarChar,4),
        ...
        paras[1].Value = fieldStr;
        paras[2].Value = orderStr;
        paras[3].Value = orderType;
        paras[4].Value = whereStr;
        paras[5].Value = pageSize;
        paras[6].Value = pageIndex;
        paras[7].Direction = ParameterDirection.Output;
       
        DataSet ds= ExecuteDataset(GetConnection(), CommandType.StoredProcedure, "Pagination", paras);
        //output 参数只有在存储过程执行后才能设置值
        totalRecord = Convert.ToInt32(paras[7].Value);
        return ds;
    }
